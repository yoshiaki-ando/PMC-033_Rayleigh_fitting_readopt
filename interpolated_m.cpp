...
std::complex <double> interpolated_m(double wavelength_in_m){
  ...
  /* from https://atmos.washington.edu/ice_optical_constants/IOP_2008_ASCIItable.dat */
  constexpr int num = 486;
  constexpr double numerical_m[num][3] = { { 4.430E-002, 0.8228, 1.640E-001 },
  ...
  const double lambda = wavelength_in_m * 1e6; /* 波長 [μm] */

  /* 波長の範囲の確認 */
  if ( (lambda < numerical_m[0][0]) ||
      (lambda > numerical_m[num-1][0]) ){
    std::cout << "Error (interpolated_m): wavelength(" << lambda
        << " [um]) is out of range." << std::endl;
    exit(1);
  }
  ...
  while(span > 1){
//    std::cout << lambda << " : " << numerical_m[middle][0] << " -> ";
    if ( numerical_m[middle][0] < lambda ){
      lower = middle;
    } else {
      upper = middle;
    }
    ...
    span = (upper - lower);
//    std::cout << lower << "-" << upper << ", span = " << span << std::endl;
  }

  /* 探索できた区間から線形補間 */
  double mc[2];
  for(int i = 0; i < 2; i++){
    mc[i] = (numerical_m[upper][i+1] - numerical_m[lower][i+1])
        /   (numerical_m[upper][0]   - numerical_m[lower][0])
        * (lambda - numerical_m[lower][0])
        + numerical_m[lower][i+1];
  }

  return std::complex <double> (mc[0], -mc[1]);
}
